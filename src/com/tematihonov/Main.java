...
public class Main {
    ...
    private static boolean checkWin(char symbol) {
        ...
        // дополнительные проверки для диагоналей [0][0]-[SIZE][SIZE]
        quantitySymbolInRow = 0;
        ...
        // дополнительные проверки для диагоналей [SIZE][0]-[0][SIZE]
        quantitySymbolInRow = 0;
        for (int j = 1; j < SIZE-DOTS_TO_WIN+1; j++) {
            int b = 0;
            for (int i = SIZE; i < 1; i--,b++) {
                if (map[b][i-j] == symbol) {
                    quantitySymbolInRow++;
                } else if (quantitySymbolInRow > 0) {
                    quantitySymbolInRow = 0;
